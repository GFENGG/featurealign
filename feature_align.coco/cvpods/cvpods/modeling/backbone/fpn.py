...
from cvpods.modeling.backbone.trans.transfor import TransformerEncoderLayer, TransformerEncoder
from cvpods.modeling.backbone.trans.position_encoding import build_position_encoding
import torch
...
class FPN(Backbone):
    ...
    def __init__(
        ...
        '''
        adding configs of transformer
        '''
        self.transformer_isuse = False

        
        if self.transformer_isuse:
            in_channels_ = [input_shapes[f].channels for f in in_features]
            trans_in_channels = in_channels_[-1]
            trans_nhead = 8
            trans_dim_feedforward = 2048
            trans_dropout = 0.1
            trans_activation="relu"
            trans_normalize_before=False
            trans_num_encoder_layers = 3
      
            self.inner_trans = TransformerEncoderLayer(trans_in_channels, trans_nhead, trans_dim_feedforward,
                                                    trans_dropout, trans_activation, trans_normalize_before)
            self.trans_normalize_before = False
            encoder_norm = nn.LayerNorm(trans_in_channels) if self.trans_normalize_before else None
            self.encoder = TransformerEncoder(self.inner_trans, trans_num_encoder_layers, encoder_norm)

            for module_p in [self.inner_trans.parameters(), self.encoder.parameters()]:
                for p in module_p:
                    if p.dim() > 1:
    ...
    def forward(self, x_with_mask):
        ...
        x = x_with_mask.tensor
        bottom_up_features = self.bottom_up(x)
        x = [bottom_up_features[f] for f in self.in_features[::-1]]
        
        if self.transformer_isuse:
            bs_res3, c_res3, h_res3, w_res3 = x[0].shape
            position_embedding = build_position_encoding(c_res3, 'sine')

            x_mask = x_with_mask.mask
            x_mask = F.interpolate(x_mask[None].float(), size=x[0].shape[-2:]).to(torch.bool)[0].to(x[0].device)
            pos = position_embedding(x[0], x_mask).to(x[0].dtype)
            x_trans = x[0].flatten(2).permute(2, 0, 1) #[bs, c, h, w] -> [bs, c, h*w] -> [hw, bs, c]
            pos = pos.flatten(2).permute(2, 0, 1) 
            x_mask = x_mask.flatten(1)
            #x[0] = self.inner_trans(x[0], src_key_padding_mask = x_mask, pos = pos) # one layer
            x_trans = self.encoder(x_trans, src_key_padding_mask = x_mask, pos = pos) # 6 layers

            x_trans = x_trans.permute(1, 2, 0).view(bs_res3, c_res3, h_res3, w_res3)
            x[0] = x_trans
...
def build_retinanet_fpn_backbone(cfg, input_shape: ShapeSpec, backbone_name="resnet"):
    ...
    block_in_feature = cfg.MODEL.FPN.BLOCK_IN_FEATURES #'p5'
