...
class FeatureAlign(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.head = FeatureAlignHead(cfg, feature_shapes)
        ...
        self.mus = [nn.Parameter(torch.zeros(80, 2)).to(self.device) for _ in range(5)]
        self.sigmas = [nn.Parameter(torch.ones(80, 2)).to(self.device) for _ in range(5)]
    ...
    def forward(self, batched_inputs, inner_iter = 0):
        """
        Args:
    ...
    def losses(self, shifts, gt_instances, box_cls, box_delta, box_center):
        ...
        for shifts_per_image, gt_instances_per_image, \
            ...
            for stride, shifts_i in zip(self.fpn_strides, shifts_per_image):
                ...
                normal_probs.append(
                    normal_distribution(distances / stride,
                                        self.mus[cnt][labels].unsqueeze(1),
                                        self.sigmas[cnt][labels].unsqueeze(1)))
                cnt += 1
            ...
            difference_w = 2 / ( 2 - (composed_cls_prob.permute(1, 0) - pred_reg_probs) ** 2 ) 
            input_logits = (composed_cls_prob.permute(1, 0) * pred_reg_probs) /  difference_w
            positive_losses.append(
                positive_bag_loss(input_logits,
            ...
            with torch.no_grad():
                gaussian_logits = composed_cls_prob.permute(1, 0) * pred_reg_probs 
                gaussian_weight = (gaussian_logits[foreground_idxs]).exp()
                for i in range(len(gt_instances_per_image)):
                    idxs = foreground_idxs[0] == i
                    if idxs.sum() > 0:
                        gaussian_weight[idxs] = normalize(gaussian_weight[idxs])

            gaussian_norm_losses.append(len(gt_instances_per_image) / (torch.sqrt((gaussian_weight * normal_probs[foreground_idxs]).clamp_(1e-12)).sum() + 1e-6))
    ...
    def inference(self, box_cls, box_delta, box_center, shifts, images):
        """
        Arguments:
            box_cls, box_delta, box_center: Same as the output of :meth:`FeatureAlignHead.forward`
...
class DFConv2d(torch.nn.Module):
    ...
    def __init__(
        ...
        if with_modulated_dcn:
            from defor.deform_conv_module import ModulatedDeformConv
            offset_channels = offset_base_channels * 3  # default: 27
            conv_block = ModulatedDeformConv
        else:
            from defor.deform_conv_module import DeformConv
...
class FeatureAlignHead(nn.Module):
    ...
    def __init__(self, cfg, input_shape: List[ShapeSpec]):
        super(FeatureAlignHead, self).__init__()
        # fmt: off
